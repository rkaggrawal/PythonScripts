...
class JSON(object):
    ...
    def encode_boolean(self, bval, state):
        ...
        state.insertLast('true' if bool(bval) else 'false')
    ...
    def encode_number(self, n, state):
        ...
        if isinstance(n, json_int):
            state.insertLast(n.json_format())
            return

        if isinstance(n, (int,long)):
            state.insertLast(str(n))
            return

        if decimal and isinstance(n, decimal.Decimal):
            if n.is_nan():  # Could be 'NaN' or 'sNaN'
                state.insertLast('NaN')
            elif n.is_infinite():
                if n.is_signed():
                    state.insertLast('-Infinity')
                else:
                    state.insertLast('Infinity')
            else:
                s = str(n).lower()
    ...
    def encode_composite(self, obj, state, obj_classification=None):
        ...
        if it is not None:
            ...
            state.insertLast(opener)
            state.insertLast(spaces_after_opener)
            ...
            if not compactly:
                if numitems > self.options.max_items_per_line:
                    ...
                    state.insertLast(' ')
            state.insertLast(closer)  # final '}' or ']'
